...
def config():
    ...
    TRIPLES = Path(TRIPLES_ROOT, f"train_triples_normal+fake_factor2_nway128_evidence+anserini.jsonl")
    QUERIES = Path(QUERIES_ROOT, f"train_normal+fake_factor2_queries.jsonl")

    DATESTR = datetime.now().strftime("%y%m%d_%H%M%S")
    EXPERIMENT = Path(COLBERT_ROOT, "checkpoints", CHECKPOINT_NAME, f"nway{NWAY}_anserini_{DATESTR}")

    NOTE = "LREV EnFEVER extended by fake claims with factor 2. Anserini and evidence."
